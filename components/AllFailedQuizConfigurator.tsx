...
interface FailedQuizConfiguratorProps {
  onStart: () => void;
  onCancel: () => void;
  failedQuestionsCount: number;
}

const AllFailedQuizConfigurator: React.FC<FailedQuizConfiguratorProps> = ({ onStart, onCancel, failedQuestionsCount }) => {
  const hasQuestions = failedQuestionsCount > 0;
        ...
        {hasQuestions ? (
          <div>
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-sans max-w-lg mx-auto">
              Tienes <span className="font-bold text-slate-700 dark:text-slate-100">{failedQuestionsCount}</span> pregunta(s) fallada(s) en total. ¡Es hora de convertirlas en aciertos!
            </p>
            <button
              onClick={onStart}
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-8 font-sans">
            ¡Enhorabuena! No tienes ninguna pregunta en tu historial de fallos.
          </p>
        )}
      </div>
    </div>
...
export default AllFailedQuizConfigurator;
