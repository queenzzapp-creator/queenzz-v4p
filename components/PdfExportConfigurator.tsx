...
const ItemCheckbox: React.FC<{
    ...
    const iconColor = isFolder ? 'text-lime-500' : 'text-slate-500';

    if (item.type === 'deck') return null; // No decks in PDF export
...
const PdfExportConfigurator: React.FC<PdfExportConfiguratorProps> = ({ library, activeLibraryName, onExport, onCancel }) => {
  const defaultFileName = `${activeLibraryName.replace(/\s+/g, '_')}_Test.pdf`;
  ...
  const sortedLibrary = useMemo(() => libraryService.sortLibraryItems(quizzesAndFolders), [quizzesAndFolders]);
  ...
  const [openFolders, setOpenFolders] = useState<Set<string>>(allItemIds);
      ...
      <main className="flex-grow flex flex-col lg:flex-row gap-8 min-h-0">
          ...
          <div className="lg:w-[60%] xl:w-2/3 flex flex-col space-y-4 h-full min-h-0">
              ...
              <div className="flex-grow overflow-y-auto p-3 bg-white/50 dark:bg-slate-800/50 rounded-md border border-slate-200 dark:border-slate-700 min-h-0">
                  {sortedLibrary.length > 0 ? renderItemTree(sortedLibrary) : <p className="text-sm text-center text-slate-500">No hay tests para exportar.</p>}
              </div>
          </div>
