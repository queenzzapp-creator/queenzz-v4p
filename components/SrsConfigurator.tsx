...
import { SparklesIcon, EyeIcon, ArrowPathIcon, BookOpenIcon } from './Icons';

interface SrsConfiguratorProps {
  dueQuestions: QuizQuestion[];
  onStartQuiz: (questions: QuizQuestion[]) => void;
  onCancel: () => void;
}

const SrsConfigurator: React.FC<SrsConfiguratorProps> = ({ dueQuestions, onStartQuiz, onCancel }) => {
   const topics = useMemo(() => {
    const topicSet = new Set<string>();
    dueQuestions.forEach(q => {
      if (q.sourceQuizTitle) {
        topicSet.add(q.sourceQuizTitle);
      }
    });
  ...
  const filteredQuestions = useMemo(() => {
    ...
    return dueQuestions.filter(q => selectedTopics.has(q.sourceQuizTitle || ''));
  }, [dueQuestions, selectedTopics]);
  
  const [questionCount, setQuestionCount] = useState(0);

  useEffect(() => {
    setQuestionCount(Math.min(filteredQuestions.length, 20));
  }, [filteredQuestions]);
  ...
  const handleCountChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (count < 1 && filteredQuestions.length > 0) count = 1;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (questionCount > 0) {
      const questionsForQuiz = [...filteredQuestions].sort(() => Math.random() - 0.5).slice(0, questionCount);
      onStartQuiz(questionsForQuiz);
    }
  };
    ...
    <div className="animate-fade-in w-full max-w-3xl mx-auto flex flex-col h-full">
      <div className="flex-shrink-0">
        ...
        <p className="text-slate-500 dark:text-slate-400 mb-8 font-sans">
          Tienes <span className="font-bold text-slate-700 dark:text-slate-100">{dueQuestions.length}</span> pregunta(s) pendiente(s) de repaso. Elige cuántas quieres practicar para afianzar tu memoria.
        </p>
      </div>

      {dueQuestions.length > 0 ? (
        <form onSubmit={handleSubmit} className="flex-grow flex flex-col space-y-6 bg-white/50 dark:bg-slate-800/50 p-8 rounded-lg border border-slate-200 dark:border-slate-700 min-h-0">
            <div className="flex-grow overflow-y-auto pr-4 -mr-4 space-y-6 pb-4">
              <div>
                <h3 className="text-lg font-semibold text-slate-700 dark:text-slate-200 mb-3">Filtrar por Tema</h3>
              ...
              <div>
                <label htmlFor="question-count" className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2 font-sans">
                  Número de preguntas a practicar ({filteredQuestions.length} disponibles)
                </label>
                <input
                  id="question-count"
                  type="number"
            ...
            <div className="flex-shrink-0 mt-auto pt-6 border-t border-slate-200 dark:border-slate-700">
                <div className="flex justify-end gap-4">
                    <button
                    type="submit"
                    disabled={questionCount === 0 || filteredQuestions.length === 0}
        ...
        <div className="text-center py-12 px-6 bg-slate-50/70 dark:bg-slate-800/40 rounded-lg border border-dashed border-slate-200 dark:border-slate-700">
            <p className="text-slate-500 dark:text-slate-400 font-sans">¡Felicidades! No tienes ninguna pregunta pendiente de repaso por hoy. Vuelve mañana para seguir aprendiendo.</p>
            <button
              onClick={onCancel}
...
export default SrsConfigurator;