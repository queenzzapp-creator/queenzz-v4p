...
interface LibraryItemComponentProps {
    ...
    onToggleMultiSelect: (item: LibraryItem, event: React.MouseEvent) => void;
             ...
             {isMultiSelectActive && (
                <div className="flex-shrink-0">
                    <input
                        ...
                        checked={isSelected}
                        readOnly
                        className="h-5 w-5 rounded-sm bg-slate-100 border-slate-300 text-sky-600 focus:ring-sky-500 pointer-events-none"
                    />
                </div>
            )}
                ...
                <div className="flex items-center gap-2">
                    {isEditing ? (
                        <input
                            ...
                            className="w-full bg-white dark:bg-slate-800 text-slate-800 dark:text-slate-100 border border-sky-500 rounded p-1 text-sm font-sans"
                        />
                    ) : (
...
const QuizLibrary: React.FC<QuizLibraryProps> = ({
    ...
    const allLibraryIds = useMemo(() => new Set(libraryService.flattenItems(library).map(i => i.id)), [library]);
    ...
    }, [selectedIds, onDeleteSelected, handleClearSelection]);
    ...
    const sortItems = useCallback((items: LibraryItem[], order: 'default' | 'az' | 'za'): LibraryItem[] => {
        ...
        const sorted = [...items].sort((a, b) => {
            ...
            const compareResult = nameA.localeCompare(nameB, 'es', { numeric: true, sensitivity: 'base' });
    ...
    const handleToggleMultiSelect = (item: LibraryItem, event: React.MouseEvent) => {
        const newSelection = new Set(selectedIds);
        const isCurrentlySelected = newSelection.has(item.id);
    
        if (event.shiftKey && lastClickedId && lastClickedId !== item.id) {
            ...
            if (lastIndex !== -1 && currentIndex !== -1) {
                ...
                for (let i = start; i <= end; i++) {
                    if (shouldSelect) {
                        ...
                        newSelection.delete(flatItems[i].id);
                    }
                }
            }
        } else {
    ...
    const handleContextMenu = useCallback((e: React.MouseEvent, item?: LibraryItem) => {
        ...
        const singleItem = currentSelection.size === 1 ? findItem(library, Array.from(currentSelection)[0]) : null;
        let menuItems: ContextMenuItem[] = [];
    
        if (singleItem) {
            menuItems.push({ label: 'Renombrar', icon: <PencilSquareIcon className="h-4 w-4" />, action: () => setEditingItemId(singleItem.id) });
            if(singleItem.type === 'quiz'){
                menuItems.push({ label: 'Editar contenido', icon: <WrenchScrewdriverIcon className="h-4 w-4" />, action: () => onEdit(singleItem as SavedQuiz) });
            }
        }
        ...
        if (currentSelection.size > 0) {
            ...
            menuItems.push({ label: `Eliminar (${currentSelection.size})`, icon: <TrashIcon className="h-4 w-4" />, action: handleDeleteSelected, className: 'text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-900/50' });
    ...
    }, [selectedIds, library, onEdit, handleDeleteSelected, setEditingItemId, setIsMoveModalOpen, setIsCreatingFolder]);
    ...
    return (
        <div className="animate-fade-in w-full max-w-7xl mx-auto flex flex-col h-full">
            ...
            <div className="flex-grow bg-white/40 dark:bg-slate-800/30 p-4 rounded-lg border border-slate-200/80 dark:border-slate-700/60 min-h-[300px] flex flex-col relative">
                <div className="flex-shrink-0">
                    {isMultiSelectActive ? (
                        <div className="flex items-center justify-between gap-2 sm:gap-4 p-2 mb-4 rounded-lg bg-sky-100 dark:bg-sky-900/40 border border-sky-200 dark:border-sky-700 animate-fade-in">
                            <div className="flex items-center gap-2">
                                <span className="text-sm font-bold px-2 text-sky-800 dark:text-sky-200">{selectedIds.size} seleccionado(s)</span>
                                <button type="button" onClick={selectedIds.size === allLibraryIds.size ? handleClearSelection : handleSelectAll} className="text-xs font-medium text-sky-600 dark:text-sky-400 hover:underline">
                                    {selectedIds.size === allLibraryIds.size ? 'Deseleccionar todo' : 'Seleccionar todo'}
                                </button>
                            </div>
                            <div className="flex items-center gap-1">
                                <button title="Mover" onClick={() => setIsMoveModalOpen(true)} className="p-2 rounded-md text-slate-600 dark:text-slate-300 hover:bg-sky-200 dark:hover:bg-sky-700/50 transition-colors"><FolderPlusIcon className="h-5 w-5" /></button>
                                <button title="Editar Contenido" disabled={!singleSelectedItem || singleSelectedItem.type !== 'quiz'} onClick={() => onEdit(singleSelectedItem as SavedQuiz)} className="p-2 rounded-md text-slate-600 dark:text-slate-300 hover:bg-sky-200 dark:hover:bg-sky-700/50 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"><PencilSquareIcon className="h-5 w-5" /></button>
                            ...
                            <button title="Cancelar Selección" onClick={handleClearSelection} className="p-1.5 rounded-full bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 hover:bg-slate-300 dark:hover:bg-slate-600 transition-colors">
