...
import { SparklesIcon, ArrowPathIcon, QuestionMarkCircleIcon } from './Icons';

interface UnansweredQuizConfiguratorProps {
  onStart: () => void;
  onCancel: () => void;
  unansweredQuestionsCount: number;
}

const UnansweredQuizConfigurator: React.FC<UnansweredQuizConfiguratorProps> = ({ onStart, onCancel, unansweredQuestionsCount }) => {
  const hasQuestions = unansweredQuestionsCount > 0;
      ...
      <div className="flex-shrink-0">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl sm:text-3xl font-bold text-slate-800 dark:text-slate-100 flex items-center gap-3">
            <QuestionMarkCircleIcon className="h-8 w-8 text-sky-500" />
            Repasar "En Blanco"
          </h2>
          <button
            onClick={onCancel}
        ...
        {hasQuestions ? (
          <div>
            <p className="text-slate-500 dark:text-slate-400 mb-8 font-sans max-w-lg mx-auto">
              Tienes <span className="font-bold text-slate-700 dark:text-slate-100">{unansweredQuestionsCount}</span> pregunta(s) que dejaste en blanco. ¡Vamos a repasarlas!
            </p>
            <button
              onClick={onStart}
          ...
          <p className="text-slate-500 dark:text-slate-400 mb-8 font-sans">
            ¡Genial! No tienes preguntas pendientes que hayas dejado en blanco.
          </p>
        )}
      </div>
    </div>
...
export default UnansweredQuizConfigurator;
