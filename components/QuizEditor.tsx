...
const QuizEditor: React.FC<QuizEditorProps> = ({ quiz, onSave, onCancel, onViewSource, mnemonicsByQuestionId, onViewMnemonic, onAddMoreQuestions }) => {
  ...
  const handleQuestionChange = (index: number, field: keyof QuizQuestion, value: string | string[] | undefined) => {
    const newQuestions = [...editedQuiz.questions];
    if (value === undefined) {
      delete (newQuestions[index] as any)[field];
    } else {
  ...
  const handleExplanationImageChange = async (e: React.ChangeEvent<HTMLInputElement>, qIndex: number) => {
    if (e.target.files && e.target.files[0]) {
        const base64 = await fileToBase64(e.target.files[0]);
        handleQuestionChange(qIndex, 'explanationImageUrl', base64);
    }
  };

  const handleExplanationPaste = async (e: React.ClipboardEvent<HTMLTextAreaElement>, qIndex: number) => {
                  ...
                  <div className="font-sans">
                   ...
                   <div className="font-sans">
                    ...
                    <textarea
                      ...
                      rows={2}
                      value={q.explanation}
                      onPaste={(e) => handleExplanationPaste(e, qIndex)}
                     ...
                     <div className="mt-2">
                        ...
                        {q.explanationImageUrl && (
                            <div className="relative group w-fit mb-2">
                                <img src={q.explanationImageUrl} alt="Previsualización" className="max-h-24 rounded border border-slate-300 dark:border-slate-600" />
                                <button type="button" onClick={() => handleQuestionChange(qIndex, 'explanationImageUrl', undefined)} className="absolute top-1 right-1 p-0.5 bg-black/50 text-white rounded-full opacity-0 group-hover:opacity-100"><XMarkIcon className="h-3 w-3" /></button>
                            </div>
                        )}
                        <input type="file" accept="image/*" onChange={(e) => handleExplanationImageChange(e, qIndex)} className="text-xs file:mr-2 file:py-1 file:px-2 file:rounded-full file:border-0 file:text-xs file:font-semibold file:bg-lime-50 file:text-lime-700 hover:file:bg-lime-100"/>
                    </div>
                  </div>
                </div>
              )
